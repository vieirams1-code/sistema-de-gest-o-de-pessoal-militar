...
## Guia prático de validação (antes x depois)

### 1) Abertura do modal de geração
- **Como era:** o botão `Gerar Períodos Automáticos` abria um modal sem escolha de escopo; a geração sempre considerava todos os militares operacionais.
- **Como deve estar:** no mesmo modal existe o campo **Escopo de geração** com seletor.

### 2) Opções de escopo
- **Como era:** não havia seleção.
- **Como deve estar:** duas possibilidades:
  - `Todos os militares operacionais` (padrão);
  - um militar específico na lista (nome + posto quando disponível).

### 3) Quantidade de processados no aviso
- **Como era:** aviso sempre indicava processamento global.
- **Como deve estar:** o aviso muda conforme seleção:
  - global: total de militares operacionais;
  - individual: `1 militar`.

### 4) Resultado da geração (mensagem de sucesso)
- **Como era:** mensagem genérica de sucesso para geração global.
- **Como deve estar:** mensagem contextual:
  - global: sucesso para todos;
  - individual: sucesso para o militar selecionado.

### 5) Resultado sem novos períodos
- **Como era:** “todos já atualizados”.
- **Como deve estar:** texto específico para o escopo:
  - global: todos atualizados;
  - individual: militar selecionado atualizado.

### 6) Cenário de erro de seleção
- **Como era:** não aplicável (não havia seleção).
- **Como deve estar:** se o militar selecionado não estiver no conjunto operacional, o sistema retorna mensagem de erro de seleção inválida.

### 7) Regra de geração (o que não mudou)
- **Permanece igual:** o cálculo de datas e janela de períodos continua o mesmo; o que mudou foi apenas o **conjunto de militares** processados por execução.
