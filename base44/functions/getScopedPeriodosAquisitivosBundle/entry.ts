...
Deno.serve(async (req) => {
  try {
    ...
    let periodosAquisitivos = []; let militares = []; let matriculasMilitar = []; let ferias = []; let registrosLivro = []; let publicacoesExOfficio = []; let contratosDesignacaoMilitar = []; let diasDescontadosFerias = []; let partialFailures = 0; let totalMilitaresEscopo = null;
    if (targetIsAdmin) {
      criteriosAplicados.add('admin');
      const [paRes,mRes,matRes,fRes,rRes,pubRes,cdmRes,ddfRes]=await Promise.allSettled([
        ...
        fetchWithRetry(() => base44.asServiceRole.entities.DiasDescontadosFerias.list('-data_desconto'),'diasDescontados.admin'),
      ]);
      for (const r of [paRes,mRes,matRes,fRes,rRes,pubRes,cdmRes,ddfRes]) if (r.status === 'rejected') partialFailures += 1;
      ...
      diasDescontadosFerias = ddfRes.status === 'fulfilled' ? (ddfRes.value || []) : [];
    } else {
      ...
      if (!militarIds || militarIds.length === 0) { warnings.push('SEM_ESCOPO'); totalMilitaresEscopo = 0; }
        ...
        const [paRes,mRes,matRes,fRes,rRes,pubRes,cdmRes,ddfRes]=await Promise.all([
          listarPorEscopoIds(base44,'PeriodoAquisitivo',militarIds,'-inicio_aquisitivo'), listarMilitaresPorIds(base44,militarIds), listarPorEscopoIds(base44,'MatriculaMilitar',militarIds,'-created_date'), listarPorEscopoIds(base44,'Ferias',militarIds,'-data_inicio'), listarPorEscopoIds(base44,'RegistroLivro',militarIds,undefined), listarPorEscopoIds(base44,'PublicacaoExOfficio',militarIds,'-created_date'), listarPorEscopoIds(base44,'ContratoDesignacaoMilitar',militarIds,'-data_inicio_contrato'), listarPorEscopoIds(base44,'DiasDescontadosFerias',militarIds,'-data_desconto')
        ]);
        periodosAquisitivos=paRes.rows; militares=mRes.rows; matriculasMilitar=matRes.rows; ferias=fRes.rows; registrosLivro=rRes.rows; publicacoesExOfficio=pubRes.rows; contratosDesignacaoMilitar=cdmRes.rows; diasDescontadosFerias=ddfRes.rows;
        if (militarIds.length > 0 && periodosAquisitivos.length > 0 && militares.length === 0) warnings.push('MILITARES_ESCOPO_NAO_CARREGADOS');
        partialFailures = paRes.partialFailures+mRes.partialFailures+matRes.partialFailures+fRes.partialFailures+rRes.partialFailures+pubRes.partialFailures+cdmRes.partialFailures+ddfRes.partialFailures;
      }
    }
    ...
    return Response.json({ periodosAquisitivos,militares,matriculasMilitar,ferias,registrosLivro,publicacoesExOfficio,contratosDesignacaoMilitar,diasDescontadosFerias,counters,meta:{ isAdmin: targetIsAdmin, modoAcesso: criteriosAplicados.size===1?Array.from(criteriosAplicados)[0]:(criteriosAplicados.size>1?'multiplo':null), userEmail: authUserEmail||null, effectiveEmail:isImpersonating?effectiveEmailNorm:null, criteriosAplicados:Array.from(criteriosAplicados), totalMilitaresEscopo, partialFailures, warnings }});
  } catch (error) {
