...
Deno.serve(async (req) => {
  try {
    ...
    let periodosAquisitivos = []; let militares = []; let matriculasMilitar = []; let ferias = []; let registrosLivro = []; let partialFailures = 0; let totalMilitaresEscopo = null;
    if (targetIsAdmin) {
      criteriosAplicados.add('admin');
      const [paRes,mRes,matRes,fRes,rRes]=await Promise.allSettled([
      ...
      for (const r of [paRes,mRes,matRes,fRes,rRes]) if (r.status === 'rejected') partialFailures += 1;
