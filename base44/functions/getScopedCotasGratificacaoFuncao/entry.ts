...
async function resolverAutorizacaoCanonica(base44: ReturnType<typeof createClientFromRequest>, effectiveEmail?: unknown) {
...
Deno.serve(async (req) => {
  try {
    ...
    if (!targetIsAdmin && !targetAcessos.length) {
      return Response.json({ error: 'Acesso negado: usuário sem acesso ativo.' }, { status: 403 });
    }

    const escopo = targetHasGlobalScope
      ? { isAdminScope: true, criteriosAplicados: new Set<string>(['global']), militarIds: null, estruturaIds: null, hasOnlyProprio: false }
      : await resolverEscopo(base44, targetAcessos, warnings);

    const tiposRaw = await listarTodos(base44, 'TipoGratificacaoFuncao', 'nome', CAMPOS_TIPO, 2000);
    if (escopo.hasOnlyProprio) {
      warnings.push('COTAS_ADMINISTRATIVAS_OCULTADAS_PARA_ACESSO_PROPRIO');
      return Response.json({
        cotas: [],
        totais: { autorizadas: 0, ocupadas: 0, disponiveis: 0, excedentes: 0 },
        tipos: (tiposRaw || []).map(sanitizerTipo),
        meta: { isAdmin: targetIsAdmin, hasGlobalScope: targetHasGlobalScope, modoAcesso: modoAcesso(escopo.criteriosAplicados), scopedFunction: true, readOnly: true, userEmail: authUserEmail || null, effectiveEmail: isImpersonating ? effectiveEmailNorm : null, warnings, limit, offset, hasNext: false, returned: 0 },
      });
    }

    const [cotasRaw, gratificacoesRaw] = await Promise.all([
      listarTodos(base44, 'CotaGratificacaoFuncao', '-updated_date'),
      listarTodos(base44, 'GratificacaoFuncao', '-updated_date'),
    ]);

    const cotasFiltradas = (cotasRaw || []).filter((cota) => dentroEscopoCota(cota, escopo)).filter((cota) => passaFiltrosCota(cota, filtros));
    const gratificacoesEscopo = (gratificacoesRaw || []).filter((item) => dentroEscopoGratificacao(item, escopo));
    const ocupadasPorCota = new Map<string, number>();
    for (const item of gratificacoesEscopo || []) {
      if (normalizeStatus(item?.status) !== STATUS_ATIVO) continue;
      const cotaId = normalizeId(item?.cota_gratificacao_funcao_id);
      if (!cotaId) continue;
      ocupadasPorCota.set(cotaId, (ocupadasPorCota.get(cotaId) || 0) + 1);
    }

    const enriquecidas = cotasFiltradas.map((cota) => sanitizerCota(cota, ocupadasPorCota.get(String(cota?.id || '')) || 0));
    const totais = enriquecidas.reduce((acc, cota) => {
      acc.autorizadas += toNumber(cota.quantidade_autorizada);
      acc.ocupadas += toNumber(cota.ocupadas);
      acc.disponiveis += toNumber(cota.disponiveis);
      acc.excedentes += toNumber(cota.excedentes);
      return acc;
    }, { autorizadas: 0, ocupadas: 0, disponiveis: 0, excedentes: 0 });

    const pagina = enriquecidas.slice(offset, offset + limit + 1);
    const hasNext = pagina.length > limit;
    const cotas = hasNext ? pagina.slice(0, limit) : pagina;

    return Response.json({
      cotas,
      totais,
      tipos: (tiposRaw || []).map(sanitizerTipo),
      meta: { isAdmin: targetIsAdmin, modoAcesso: modoAcesso(escopo.criteriosAplicados), scopedFunction: true, readOnly: true, userEmail: authUserEmail || null, effectiveEmail: isImpersonating ? effectiveEmailNorm : null, warnings, limit, offset, hasNext, returned: cotas.length },
    });
  } catch (error) {
    const status = error?.response?.status || error?.status || 500;
    return Response.json({ error: error?.message || 'Erro ao carregar cotas escopadas de Gratificação de Função.', meta: { status } }, { status });
  }
});
