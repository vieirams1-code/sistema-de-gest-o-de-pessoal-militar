...
import {
  STATUS_FERIAS_IMPACTO_PLANO,
  STATUS_FERIAS_PREVISAO_PLANO,
  numeroSeguro,
  textoId,
  adicionarDiasData,
  feriasVinculadasAoPlano,
  calcularResumoPeriodoPlano,
  normalizarPreferenciaMes,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ========================================================================
    const isCadastroGestaoAction = Boolean(
    ...
    const isMilitarPortalAction = Boolean(
      ...
      (acao?.startsWith('CADASTRO_') && !isCadastroGestaoAction) ||
      ...
      switch (acao) {
        ...
        case 'CAMPANHA_EDITAR': {
          ...
          if (campanhaAtual.tipo === 'PLANO_FERIAS' || campanhaAtual.plano_ferias_institucional_id) {
            return new Response(JSON.stringify({ error: 'Campanhas de Planos de Férias devem ser editadas pelo módulo de Planos de Férias.' }), {
              status: 403,
              headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' },
            });
          }
        ...
        case 'PLANO_INSTITUCIONAL_GERAR_FERIAS': {
          ...
          if (acao === 'PLANO_GERAR_LOTE_FERIAS' || acao === 'PLANO_INSTITUCIONAL_GERAR_FERIAS') {
            const { gerarFeriasPlano } = await import('../../shared/ferias/gerarFeriasPlano.ts');
            return await gerarFeriasPlano({ base44, user, payload, acao, calcularResumoPeriodoPlano, consolidarOpcoesPlano, registrarAuditoriaFerias, corsHeaders: CORS_HEADERS });
          }
          break;
        }
    ...
    switch (acao) {
      ...
      case 'FERIAS_GET': {
        const { feriasGet } = await import('../../shared/ferias/portalFeriasServicos.ts');
        return await feriasGet({
          base44,
          req,
          payload,
          militar,
      ...
      case 'FERIAS_SUBMETER_OPCAO': {
        const { feriasSubmeterOpcao } = await import('../../shared/ferias/portalFeriasServicos.ts');
        return await feriasSubmeterOpcao({
          base44,
          req,
          payload,
          militar,
