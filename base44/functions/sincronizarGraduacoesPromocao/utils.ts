...
export async function atualizarCadastroMilitar(
  ...
  if (!militarAntes) throw new Error(`Militar ${militarId} não encontrado para atualização.`);
  ...
  const updates = comparativos.map((comp) => {
  ...
  const success = !erro_api && updates.every((item) => item.confirmado);
  ...
  if (success) {
    await AssistenteLog.create({
      ...
      metadata: {
        ...
        campos_afetados: updates.map((item) => item.campo),
      },
    });
  }

  return { militar_id: militarId, matricula, success, updates, erro_api };
}
