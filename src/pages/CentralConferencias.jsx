      ...
      {nova && <>
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="flex items-center justify-between gap-3 border-t pt-4"><p className="text-sm text-slate-500">Universo atual: <b>{universo.length}</b> militar(es) · Entrada: <b>{linhas.length}</b> linha(s)</p><div className="flex gap-2"><Button variant="outline" onClick={() => { setNova(false); setEditandoId(''); setBuscaItens(''); }}>Cancelar</Button><Button onClick={executar}><ClipboardCheck className="w-4 h-4 mr-2" />{editandoId ? 'Recalcular conferência' : 'Conferir lista'}</Button></div></div>
          </CardContent>
        </Card>
