...
export default function CadastrarTarefaOperacional() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      await base44.entities.TarefaOperacional.create(payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tarefas-operacionais'] });
      toast({
        title: 'Tarefa criada',
        description: 'A tarefa operacional foi criada com sucesso.',
      });
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <header className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">LOTE 1 - cadastro inicial da tarefa para gestão operacional.</p>
          </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2 md:col-span-2">
                ...
                <Input id="titulo" value={formData.titulo} onChange={(e) => handleChange('titulo', e.target.value)} placeholder="Ex: Conferência de armamento semanal" />
              </div>
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Textarea id="descricao" rows={6} value={formData.descricao} onChange={(e) => handleChange('descricao', e.target.value)} placeholder="Descreva a orientação para execução da tarefa." />
              </div>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 border-t border-slate-100 pt-4">
              <label className="flex items-start gap-3 rounded-lg border border-slate-200 p-3 cursor-pointer">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Obrigará envio de anexo nas próximas etapas do módulo.</p>
                </div>
              </label>
