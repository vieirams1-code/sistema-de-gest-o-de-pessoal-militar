    ...
    try {
      ...
      await base44.functions.invoke('portal_servicos', {
        ...
        campanha_payload: {
          ...
          hora_fim_militar: campanhaForm.hora_fim_militar || '23:59',
      ...
      {modalCampanha && campanhaForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <form onSubmit={salvarCampanha} className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl space-y-4">
            ...
            <div className="grid sm:grid-cols-4 gap-3"><div><label className="text-xs font-bold text-slate-700">Início *</label><Input required type="date" value={campanhaForm.data_inicio} onChange={(e) => setCampanhaForm({ ...campanhaForm, data_inicio: e.target.value })} /></div><div><label className="text-xs font-bold text-slate-700">Prazo militar *</label><Input required type="date" value={campanhaForm.data_fim_militar} onChange={(e) => setCampanhaForm({ ...campanhaForm, data_fim_militar: e.target.value })} /></div><div><label className="text-xs font-bold text-slate-700">Hora limite *</label><Input required type="time" value={campanhaForm.hora_fim_militar || '23:59'} onChange={(e) => setCampanhaForm({ ...campanhaForm, hora_fim_militar: e.target.value })} /></div><div><label className="text-xs font-bold text-slate-700">Prazo unidade</label><Input type="date" value={campanhaForm.data_fim_unidade} onChange={(e) => setCampanhaForm({ ...campanhaForm, data_fim_unidade: e.target.value })} /></div></div>
            <div><label className="text-xs font-bold text-slate-700">Escopo da campanha</label><select disabled={Boolean(campanhaForm.escopo_militares_ids?.length)} value={campanhaForm.tipo_escopo} onChange={(e) => { const tipo = e.target.value; setCampanhaForm({ ...campanhaForm, tipo_escopo: tipo, escopo_unidades_ids: tipo === 'UNIDADES' || tipo === 'UNIDADES_E_GRUPOS' ? (campanhaForm.escopo_unidades_ids || []) : [], escopo_grupos_ids: tipo === 'SEM_ESCOPO' || tipo === 'UNIDADES_E_GRUPOS' ? (campanhaForm.escopo_grupos_ids || []) : [], escopo_militares_ids: tipo === 'SELECAO_MILITARES' ? (campanhaForm.escopo_militares_ids || []) : [] }); }} className="mt-1 h-10 w-full rounded-xl border border-slate-300 px-3 text-sm"><option value="TODOS">Toda a corporação</option>{Boolean(campanhaForm.escopo_militares_ids?.length) && <option value="SELECAO_MILITARES">Seleção de militares</option>}<option value="UNIDADES">Somente unidades selecionadas</option><option value="SEM_ESCOPO">Somente Grupo de Militares (sem lotação)</option><option value="UNIDADES_E_GRUPOS">Unidades + Grupos (combinados)</option></select>{campanhaForm.tipo_escopo === 'SELECAO_MILITARES' && <p className="mt-2 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-800">{(campanhaForm.escopo_militares_ids || []).length} militar(es) selecionado(s) a partir da cobertura do plano.</p>}{campanhaForm.tipo_escopo === 'SEM_ESCOPO' && <p className="mt-1 text-[11px] text-slate-500">A elegibilidade será definida somente pelos grupos selecionados.</p>}{campanhaForm.tipo_escopo === 'UNIDADES_E_GRUPOS' && <p className="mt-1 text-[11px] text-slate-500">A elegibilidade será definida pela combinação das unidades e dos grupos selecionados.</p>}</div>
            ...
            {campanhaForm.tipo_escopo !== 'SELECAO_MILITARES' && <div>
        ...
        <div className="max-w-6xl mx-auto space-y-5">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {modoAdmin && podeAdminFerias && podeExcluirPlanos && selecionado.status === 'ARQUIVADO' && <Button type="button" variant="outline" onClick={() => excluir(selecionado)} disabled={salvando} className="border-red-200 text-red-700 hover:bg-red-50"><Trash2 className="w-4 h-4 mr-1.5" />Excluir plano</Button>}
              {(podeVisualizarRespostas || podeGerarFerias || podeAdminFerias) && <Button type="button" onClick={() => navigate('/PainelPlanoFeriasV2?planoId=' + selecionado.id)} className="bg-blue-700 hover:bg-blue-800"><CalendarDays className="w-4 h-4 mr-1.5" />Abrir painel consolidado</Button>}
              {podeEditarPlanos && selecionado.status !== 'ARQUIVADO' && <Button type="button" variant="outline" onClick={() => abrirEdicao(selecionado)}><Edit3 className="w-4 h-4 mr-1.5" />Editar plano</Button>}
                  ...
                  <div key={campanha.id} className={`p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 ${campanha.status === 'Arquivada' ? 'bg-slate-50 border-l-4 border-slate-400' : campanha.status === 'Desativada' ? 'bg-amber-50/40 border-l-4 border-amber-300' : 'bg-white'}`}>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {campanha.status === 'Encerrada' && <span className="rounded-full bg-rose-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-rose-800">Encerrada</span>}
                      </div>
                      <p className="text-xs text-slate-500 mt-1">Escopo: {campanha.tipo_escopo === 'SEM_ESCOPO' ? 'Somente Grupo de Militares' : campanha.tipo_escopo === 'UNIDADES_E_GRUPOS' ? 'Unidades + Grupos' : (campanha.escopo_unidades_nomes || 'Toda a Corporação')}{campanha.escopo_grupos_nomes ? ` · Grupos: ${campanha.escopo_grupos_nomes}` : ''} · Prazo: {campanha.data_fim_militar || '-'}{campanha.hora_fim_militar ? ` às ${campanha.hora_fim_militar}` : ''}{campanha.quantidade_prorrogacoes ? ` · ${campanha.quantidade_prorrogacoes} prorrogação(ões)` : ''}</p>{campanha.data_fim_militar_original && <p className="text-[11px] text-slate-400 mt-1">Prazo original: {campanha.data_fim_militar_original}{campanha.hora_fim_militar_original ? ` às ${campanha.hora_fim_militar_original}` : ' · hora não registrada'}</p>}
                    </div>
                    <div className="flex gap-2 flex-wrap">
