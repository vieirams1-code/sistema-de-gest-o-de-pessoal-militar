...
export default function InativacaoPeriodoLote() {
  ...
  const carregarPrevia = async () => {
    ...
    setResultado(null);
    try {
      setPrevia(await chamar({ acao: 'PREVIA' }));
    } catch (error) {
  ...
  const confirmarInativacao = async () => {
    ...
    const confirmado = window.confirm(
      `Confirmar a inativação do período 24/25 de ${previa.alvos.length} militar(es)? Esta ação altera dados de produção.`
    );
    if (!confirmado) return;
    setExecutando(true);
    try {
      const data = await chamar({ acao: 'EXECUTAR' });
      setResultado(data);
      setPrevia(null);
  ...
  const reverter = async () => {
    const confirmado = window.confirm(
      'Reativar o período 24/25 de todos os militares afetados por esta operação?'
    );
    if (!confirmado) return;
    setRevertendo(true);
    try {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Inativação do Período 24/25 em Lote</h1>
          <p className="text-sm text-slate-500">
            Inativa o período aquisitivo 2024/2025 dos militares ativos de Nova Alvorada do Sul e Sidrolândia.
          </p>
        </div>
      </div>
      ...
      <Alert className="border-amber-300 bg-amber-50">
        ...
        <AlertDescription className="text-amber-800">
          A inativação remove o período 24/25 do controle e libera o militar para preencher o plano de férias.
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-slate-600">
            ...
            <strong>Sidrolândia</strong> que possuem o período 24/25 ainda em aberto.
          </p>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-slate-600">
            Reativa o período 24/25 apenas dos militares inativados por esta operação, conforme o registro de
            auditoria. Inativações feitas por outros motivos não são afetadas.
          </p>
