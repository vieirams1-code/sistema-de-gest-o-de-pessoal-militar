      ...
      <div className="grid grid-cols-1 xl:grid-cols-[380px_1fr] gap-6">
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5"><h3 className="font-semibold text-slate-900 mb-3">Adicionar militar</h3><div className="relative mb-3"><Search className="absolute left-3 top-3 w-4 h-4 text-slate-400" /><Input className="pl-9" placeholder="Nome ou matrícula..." value={militarBusca} onChange={(e) => setMilitarBusca(e.target.value)} /></div><div className="max-h-80 overflow-auto divide-y">{militaresDisponiveis.map((m) => <div key={m.id} className="flex items-center justify-between gap-2 py-2"><div><p className="text-sm font-medium">{m.nome}</p><p className="text-xs text-slate-500">{m.matricula || m.posto_graduacao || '—'}</p></div><Button size="sm" variant="outline" onClick={() => vincular.mutate(m.id)} disabled={vincular.isPending}><UserPlus className="w-4 h-4" /></Button></div>)}{militarBusca && militaresDisponiveis.length === 0 && <p className="text-sm text-slate-500 py-3">Nenhum militar disponível.</p>}</div></div>
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5"><h3 className="font-semibold text-slate-900 mb-3">Membros do grupo</h3><div className="max-h-96 overflow-auto divide-y">{membros.filter((m) => m.ativo !== false).map((m) => { const militar = militares.find((x) => x.id === m.militar_id); return <div key={m.id} className="flex items-center justify-between gap-2 py-2"><div><p className="text-sm font-medium">{militar?.nome || 'Militar não localizado'}</p><p className="text-xs text-slate-500">{militar?.matricula || '—'}</p></div><Button size="sm" variant="ghost" onClick={() => desvincular.mutate(m.id)} disabled={desvincular.isPending}><UserMinus className="w-4 h-4 text-rose-600" /></Button></div>; })}{membros.filter((m) => m.ativo !== false).length === 0 && <p className="text-sm text-slate-500 py-3">Nenhum membro ativo.</p>}</div></div>
          </div></>}
        </section>
      </div>
    </div>
