...
import { getMilitarTagMilitarId, getMilitarTagTagId, isRegistroAtivo } from '@/utils/funcoesTags/contratoCampos';
...
export default function Militares() {
  ...
  const { data: tagsAtivas = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return tags.filter((tag) => isRegistroAtivo(tag) && APLICABILIDADE_TAG_MILITAR.has(String(tag?.aplicabilidade || '').toLowerCase()));
    },
  });
  ...
  const { data: gruposAtivos = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return grupos.filter((grupo) => isRegistroAtivo(grupo));
    },
  });
  ...
  const executarBulk = async ({ mode, tagIds, motivo }) => {
    ...
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: ['militares-tags-filtros', funcoesTagsScopeKey] }),
