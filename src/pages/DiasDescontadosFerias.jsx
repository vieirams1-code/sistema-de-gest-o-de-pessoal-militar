...
export default function DiasDescontadosFerias() {
  ...
  const { data: descontos = [], error: erroDescontos } = useQuery({ queryKey: ['dias-descontados-ferias'], queryFn: () => listarDescontosFerias(), enabled: isAccessResolved && canView && Boolean(base44?.entities?.DiasDescontadosFerias), retry: 1, refetchOnWindowFocus: true });
  ...
  const salvarPublicacaoMutation = useMutation({
    ...
    onSuccess: (res) => {
      const isAtivo = res?.desconto?.status === STATUS_DESCONTO_FERIAS.ATIVO;
      alert(isAtivo ? 'Publicação salva com sucesso e desconto de férias efetivado.' : 'Publicação salva com sucesso. O desconto será efetivado após informar os dados do BG.');
  ...
  const gerarPublicacao = () => {
    ...
    if (Number(form.dias_descontados) > resumo.saldoAtual) return setErroCadastro(`Quantidade maior que o saldo atual (${resumo.saldoAtual} dia(s)).`);
    if (Number(form.dias_descontados) > resumo.limiteRestante) return setErroCadastro(`Este período já possui ${resumo.diasDescontados} dias descontados. Restam apenas ${resumo.limiteRestante} para o limite de 8 dias.`);
  ...
  return <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
    ...
    <Card><CardContent className="p-0 overflow-x-auto"><table className="w-full text-sm"><thead className="bg-slate-100 text-slate-600"><tr>{['Militar','Período aquisitivo','Dias descontados','Publicação gerada','Status publicação','Status desconto','Data','Usuário responsável','Ações'].map(h => <th key={h} className="text-left px-4 py-3">{h}</th>)}</tr></thead><tbody>{filtrados.map((d) => {
      ...
      return <tr key={d.id} className="border-t">
        <td className="px-4 py-3 font-medium">{d.militar_posto} {d.militar_nome}<div className="text-xs text-slate-500">{d.militar_matricula}</div></td>
        <td className="px-4 py-3">{d.periodo_aquisitivo_ref}</td>
        <td className="px-4 py-3">{d.dias_descontados}</td>
        <td className="px-4 py-3">
          {d.publicacao_numero_bg ? `BG nº ${d.publicacao_numero_bg}` : (d.publicacao_status === 'Aguardando Nota' ? 'Aguardando Nota' : '—')}
        </td>
        <td className="px-4 py-3">{d.publicacao_status || 'Aguardando Nota'}</td>
        <td className="px-4 py-3"><Badge variant={d.status === 'ativo' ? 'default' : (d.status === 'pendente_publicacao' ? 'warning' : 'secondary')}>{statusLabel}</Badge></td>
        <td className="px-4 py-3">{fmt(d.data_dispensa || d.data_desconto)}</td>
        <td className="px-4 py-3">{d.usuario_criacao || '—'}</td>
        <td className="px-4 py-3">
          <div className="flex gap-2">
            {d.publicacao_id && <Button size="sm" variant="ghost" title="Ver publicação" onClick={() => navigate(createPageUrl('Publicacoes', { search: d.publicacao_id }))}><FileText className="h-3 w-3" /></Button>}
            {(d.status === 'ativo' || d.status === 'pendente_publicacao') && <Button size="sm" variant="outline" disabled={cancelarMutation.isPending} onClick={() => cancelarMutation.mutate(d)}><RotateCcw className="h-3 w-3 mr-1" />{d.status === 'ativo' ? 'Cancelar/reverter' : 'Cancelar'}</Button>}
          </div>
        </td>
      </tr>;
    })}{!filtrados.length && <tr><td colSpan="9" className="px-4 py-8 text-center text-slate-500">Nenhum desconto encontrado.</td></tr>}</tbody></table></CardContent></Card>
