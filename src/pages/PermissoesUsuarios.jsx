    ...
    <div className="min-h-screen bg-slate-100 p-4 lg:p-6">
      <div className="max-w-[1780px] mx-auto">
        <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
          ...
          <Button onClick={handleCreateNew} className="bg-[#1e3a5f] text-white hover:bg-[#2d4a6f]">
        ...
        <div className="grid grid-cols-1 gap-5 2xl:grid-cols-12 items-start">
          <div className="2xl:col-span-3 flex flex-col min-h-[calc(100vh-10rem)]">
            <div className="flex min-h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="sticky top-0 z-10 border-b border-slate-200 bg-white p-4">
                <h2 className="font-semibold text-slate-900">Usuários</h2>
                <p className="mb-3 text-xs text-slate-500">{acessos.length} registros</p>
              ...
              <div className="flex-1 space-y-2 overflow-y-auto bg-slate-50/50 p-3">
                {filteredAcessos.length === 0 ? (
                  <p className="py-10 text-center text-sm text-slate-400">Nenhum acesso cadastrado</p>
                ) : (
                  filteredAcessos.map((u) => (
                    <div
                      key={u.id}
                      onClick={() => handleSelectAcesso(u)}
                      className={`cursor-pointer rounded-xl border p-3 transition-all ${selectedUser?.id === u.id ? 'border-[#1e3a5f] bg-[#1e3a5f] text-white shadow-md ring-2 ring-[#1e3a5f]/35' : 'border-slate-200 bg-white hover:border-slate-300 hover:shadow-sm'}`}
                    >
                      <div className="mb-1.5 flex items-start justify-between gap-2">
                        <div className={`truncate text-sm font-semibold ${selectedUser?.id === u.id ? 'text-white' : 'text-slate-800'}`} title={u.nome_usuario || u.user_email}>
                          {u.nome_usuario || u.user_email}
                          {!u.ativo && (
                            <span className={`ml-1.5 text-xs font-semibold ${selectedUser?.id === u.id ? 'text-rose-100' : 'text-red-500'}`}>(Inativo)</span>
                          )}
                        </div>
                        <div className="shrink-0 [&_.bg-amber-100]:bg-white/20 [&_.bg-blue-100]:bg-white/20 [&_.bg-emerald-100]:bg-white/20 [&_.bg-emerald-50]:bg-white/20 [&_.border-emerald-200]:border-white/30 [&_.text-amber-800]:text-white [&_.text-blue-800]:text-white [&_.text-emerald-700]:text-white [&_.text-emerald-800]:text-white [&_.text-slate-500]:text-white">
                          {getTipoBadge(u)}
                        </div>
                      </div>
                      <div className={`mb-2 truncate text-xs ${selectedUser?.id === u.id ? 'text-slate-100' : 'text-slate-500'}`} title={u.user_email}>{u.user_email}</div>

                      {u.perfil_nome && (
                        <div className={`inline-block max-w-full truncate rounded-full px-2 py-0.5 text-[10px] font-medium ${selectedUser?.id === u.id ? 'border border-white/30 bg-white/20 text-white' : 'border border-indigo-100 bg-indigo-50 text-indigo-600'}`}>
          ...
          <div className="2xl:col-span-9 min-w-0">
            {selectedUser ? (
              <div className="flex min-h-[calc(100vh-10rem)] flex-col rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="sticky top-0 z-20 shrink-0 border-b border-slate-200 bg-white p-4 lg:p-5">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                      <h2 className="text-lg font-bold text-[#1e3a5f] lg:text-xl">Editando: {userNomeUsuario || userUserEmail || 'Novo Usuário'}</h2>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        className="rounded-lg bg-[#1e3a5f] text-white hover:bg-[#2d4a6f]"
                      >
                ...
                <div className="min-w-0 flex-1 space-y-5 bg-slate-50/50 p-4 lg:p-6">
                      ...
                      {topPanels.map((panel) => {
                        ...
                        return (
                          <button
                            ...
                            className={`rounded-lg px-3.5 py-2 text-sm font-medium transition ${isActive ? 'bg-[#1e3a5f] text-white shadow-sm' : 'bg-slate-50 text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}
                          >
                            {panel.label}
                          </button>
                        );
                  ...
                  {activePanel === 'dados' && (
                    <div className="rounded-xl border border-slate-200 bg-white p-4">
                      ...
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                          <label className="mb-1.5 block text-sm font-semibold text-slate-700">Nome de Usuário</label>
                          <Input value={userNomeUsuario} onChange={(e) => setUserNomeUsuario(e.target.value)} placeholder="Ex: João da Silva" className="bg-white" />
                        </div>
                        <div>
                          <label className="mb-1.5 block text-sm font-semibold text-slate-700">E-mail do Usuário <span className="text-red-500">*</span></label>
                          <Input value={userUserEmail} onChange={(e) => setUserUserEmail(e.target.value)} placeholder="email@exemplo.com" className="bg-white" />
                        </div>
                        <div className="col-span-1 flex items-center gap-2 border-t border-slate-200 pt-1 md:col-span-2">
                          <input type="checkbox" id="userAtivo" checked={userAtivo} onChange={(e) => setUserAtivo(e.target.checked)} className="h-5 w-5 rounded border-slate-300 text-[#1e3a5f]" />
                          <label htmlFor="userAtivo" className="cursor-pointer text-sm font-semibold text-slate-700">Usuário Ativo e Habilitado para Login</label>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {activePanel === 'escopo' && (
                    <div className="space-y-5">
                      <div className="rounded-xl border border-slate-200 bg-white p-4">
                        ...
                        <div className="mb-5">
                          <label className="mb-1.5 block text-sm font-semibold text-slate-700">Escopo de Acesso</label>
                          <Select
                        ...
                        {(userAccessMode === 'setor' || userAccessMode === 'subsetor' || userAccessMode === 'unidade') && (
                          <div className="grid grid-cols-1 gap-5 rounded-lg border border-blue-100 bg-white p-4 md:grid-cols-3">
                            <div>
                              <label className="mb-1.5 block text-sm font-semibold text-slate-700">Setor (Obrigatório)</label>
                              ...
                              {userAccessMode === 'setor' && userGrupamentoId && !userSubgrupamentoId && <p className="mt-2 text-xs font-medium text-blue-600">✓ Terá visão de dados deste setor inteiro.</p>}
                            </div>

                            {(userAccessMode === 'subsetor' || userAccessMode === 'unidade') && (
                              <div>
                                <label className="mb-1.5 block text-sm font-semibold text-slate-700">Subsetor/Seção (Obrigatório)</label>
                                <Select
                                ...
                                {userAccessMode === 'subsetor' && userSubgrupamentoId && <p className="mt-2 text-xs font-medium text-blue-600">✓ Terá visão de dados apenas deste subsetor e suas unidades.</p>}
                              </div>
                            )}

                            {userAccessMode === 'unidade' && (
                              <div>
                                <label className="mb-1.5 block text-sm font-semibold text-slate-700">Unidade (Obrigatório)</label>
                                <Select
                                ...
                                {userUnidadeId && <p className="mt-2 text-xs font-medium text-blue-600">✓ Terá visão estrita e limitada a esta unidade.</p>}
                              </div>
                            )}
                          </div>
                        )}
                      ...
                      {userAccessMode === 'proprio' && (
                        <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-5">
                          <div className="mb-4 flex items-start justify-between">
                            ...
                            <Badge variant="outline" className="border-indigo-200 bg-indigo-100 text-xs text-indigo-800">Modo Próprio</Badge>
                          </div>
                          ...
                          <div className="grid grid-cols-1 gap-5 rounded-lg border border-indigo-50 bg-white p-4 md:grid-cols-2">
                            <div>
                              <label className="mb-1.5 block text-sm font-semibold text-slate-700">Militar Vinculado <span className="text-red-500">*</span></label>
                            ...
                            <div>
                              <label className="mb-1.5 block text-sm font-semibold text-slate-700">E-mail Secundário do Vínculo</label>
                              ...
                              <p className="mt-1.5 text-xs font-medium text-slate-500">E-mail principal do acesso: {userUserEmail || 'sem e-mail preenchido'}</p>
                            </div>
                          </div>
                        </div>
                      )}
                  ...
                  {activePanel === 'perfil' && (
                    <div className="rounded-xl border border-slate-200 bg-white p-4">
                      ...
                      <div className="flex flex-col items-end gap-4 rounded-lg border border-slate-200 bg-white p-4 sm:flex-row">
                        <div className="w-full flex-1">
                          <label className="mb-1.5 block text-sm font-semibold text-slate-700">Carregar permissões de um Perfil existente</label>
                          <Select value={selectedProfileId} onValueChange={(v) => setSelectedProfileId(v)}>
                            ...
                            <SelectContent>
                              ...
                              {perfis.map((p) => <SelectItem key={p.id} value={p.id}>{p.nome_perfil}</SelectItem>)}
                            </SelectContent>
                          </Select>
                        </div>
                        <Button
                          ...
                          className="w-full shrink-0 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 sm:w-auto"
                        >
                          <Settings2 className="mr-2 h-4 w-4" /> Aplicar Perfil
                        </Button>
                      </div>
                      <p className="mt-3 flex items-center gap-1 text-xs text-slate-500">
                        <Info className="h-4 w-4" /> Selecionar um perfil apenas exibe a prévia abaixo. As permissões do formulário só mudam ao clicar em <b>Aplicar Perfil</b>.
                      </p>
                        ...
                        <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50/50 p-4">
                          ...
                          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                            <div className="rounded-lg border border-emerald-100 bg-white p-3">
                              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-emerald-700">Módulos do perfil</p>
                              <div className="space-y-2">
                                {modulosList.map((mod) => (
                                  <label key={`preview_mod_${mod.key}`} className="flex items-center gap-2 text-sm text-slate-700">
                                    <input type="checkbox" checked={selectedProfilePreview[mod.key] === true} readOnly className="h-4 w-4 rounded border-slate-300 text-blue-600 pointer-events-none" />
                                    <span>{mod.label}</span>
                                  </label>
                                ))}
                              </div>
                            ...
                            <div className="rounded-lg border border-orange-100 bg-white p-3">
                              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-orange-700">Ações sensíveis do perfil</p>
                              <div className="space-y-2">
                                {acoesSensiveis.map((act) => (
                                  <label key={`preview_act_${act.key}`} className="flex items-center gap-2 text-sm text-slate-700">
                                    <input type="checkbox" checked={selectedProfilePreview[act.key] === true} readOnly className="h-4 w-4 rounded border-orange-300 text-orange-600 pointer-events-none" />
                                    <span>{act.label}</span>
                                  </label>
                                ))}
                              </div>
                  ...
                  {activePanel === 'matriz' && (
                    <div className="rounded-xl border border-slate-200 bg-white p-4">
                      ...
                      <div className="grid items-start gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
                        <aside className="sticky top-24 h-fit rounded-xl border border-slate-200 bg-slate-50 p-2">
                          {permissionStructure.map((categoryGroup) => {
                            ...
                            return (
                              <button
                                ...
                                className={`mb-1 w-full rounded-lg px-3 py-2 text-left text-sm transition ${isActive ? 'bg-[#1e3a5f] text-white shadow-sm' : 'text-slate-700 hover:bg-white hover:text-slate-900'}`}
                              >
                              ...
                              <div key={mod.key} className={`rounded-xl border ${isModuleEnabled ? 'border-blue-200 bg-blue-50/40' : 'border-slate-200 bg-white'}`}>
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    ...
                                    <label className="pointer-events-none text-sm font-semibold text-slate-800">{mod.label}</label>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    {moduleOverride && <span className="rounded px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-orange-700 bg-orange-100">Modificado</span>}
                                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${isModuleEnabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
                                ...
                                {mod.actions.length > 0 && isModuleEnabled && (
                                  <div className="px-3.5 pb-3.5">
                                    <div className="grid grid-cols-1 gap-2 border-t border-blue-100 pt-3 xl:grid-cols-2">
                                      {mod.actions.map((act) => {
                                        ...
                                        return (
                                          <div
                                            key={act.key}
                                            className={`flex cursor-pointer items-center justify-between gap-3 rounded-md border p-2 ${isActionEnabled ? 'border-orange-200 bg-orange-50' : 'border-slate-200 bg-white'} ${act.sensitive ? 'ring-1 ring-orange-100' : ''}`}
                                            onClick={() => setUserPermissions((prev) => ({ ...prev, [act.key]: !prev[act.key] }))}
                                          >
                                            <div className="flex items-center gap-2">
                                              <input type="checkbox" checked={isActionEnabled} readOnly className="pointer-events-none h-4 w-4 rounded border-orange-300 text-orange-600" />
                                              <span className="text-sm text-slate-700">{act.label}</span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                              {act.sensitive && <span className="rounded bg-orange-100 px-1.5 py-0.5 text-[10px] font-bold text-orange-700">Sensível</span>}
                                              {actionOverride && <span className="rounded bg-indigo-100 px-1.5 py-0.5 text-[10px] font-bold text-indigo-700">Mod.</span>}
                                            </div>
                                          </div>
                                        );
                                      })}
              ...
              <div className="flex min-h-[520px] flex-col items-center justify-center rounded-xl border border-slate-200 bg-white p-12 shadow-sm">
                <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-slate-50 text-slate-300">
                  <UserPlus className="ml-1 h-8 w-8" />
                </div>
                <h2 className="mb-2 text-lg font-bold text-slate-700">Nenhum usuário selecionado</h2>
                ...
                <Button onClick={handleCreateNew} className="bg-[#1e3a5f] text-white hover:bg-[#2d4a6f]">
                  <Plus className="mr-2 h-4 w-4" /> Cadastrar Novo Usuário
                </Button>
              </div>
            )}
          </div>
