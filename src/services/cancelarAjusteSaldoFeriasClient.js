...
export async function cancelarAjusteSaldoFerias(payload = {}) {
  ...
  const response = await base44.functions.fetch('cancelarAjusteSaldoFerias', {
    method: 'POST',
  ...
  if (!response.ok || body?.error) {
    const message = body?.error || response.statusText || 'Falha ao cancelar ajuste de saldo de férias.';
