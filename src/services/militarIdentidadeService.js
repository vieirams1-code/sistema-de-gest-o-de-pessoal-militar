...
export async function criarMilitarComMatricula(payload = {}, { origemRegistro = 'manual' } = {}) {
  ...
  if (duplicidadeForte) {
    throw new Error(`${ERROS.POSSIVEL_DUPLICIDADE} Militar relacionado: ${duplicidadeForte.nome_completo || duplicidadeForte.id}.`);
  }
...
export async function atualizarMilitarSemTrocarMatricula(militarId, payload = {}) {
  ...
  if (duplicidadeForte) throw new Error(ERROS.POSSIVEL_DUPLICIDADE);
