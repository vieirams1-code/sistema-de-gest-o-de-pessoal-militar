...
test('exclusão do militar encerra pendências de comportamento pendentes', async () => {
  const entities = {
    ...
    PendenciaComportamento: createEntity([
      ...
      { id: 'p2', militar_id: 'm1', status_pendencia: 'Aplicada', detalhes_calculo: '' },
    ]),
  };
  ...
  assert.equal(resultado.pendencias.totalTratadas, 1);
  ...
  assert.equal(p2.status_pendencia, 'Aplicada');
