...
export default function CarreiraAntiguidadePanel({ militar, historicoPromocoes, onOpenPromocaoAtualModal, onOpenPromocaoHistoricaModal, onOpenPromocaoFuturaModal, onHistoricoChanged, canManage }) {
  ...
  const promocaoAtual = React.useMemo(() => ativos.find((h) => valorTexto(h.posto_graduacao_novo) === valorTexto(militar?.posto_graduacao)
    && valorTexto(h.quadro_novo) === valorTexto(militar?.quadro)
    && valorTexto(h.data_promocao)
    && isOrdemPreenchida(h.antiguidade_referencia_ordem)) || null, [ativos, militar]);
  ...
  return <div className="space-y-4">
    ...
    <Card><CardHeader className="flex flex-row justify-between items-start gap-3"><CardTitle>Dados da Promoção Atual</CardTitle>{canManage && <div className="flex gap-2"><Button onClick={onOpenPromocaoAtualModal}>Registrar / Retificar Promoção Atual</Button><Button variant="outline" onClick={onOpenPromocaoFuturaModal}>Nova promoção futura</Button></div>}</CardHeader><CardContent className="space-y-4 text-sm">{!promocaoAtual && <div className="flex items-start gap-2 rounded-md border border-amber-300 bg-amber-50 p-3 text-amber-900"><AlertTriangle className="w-4 h-4 mt-0.5" />Sem promoção atual cadastrada.</div>}{haMultiplosRegistros && <div className="flex items-start gap-2 rounded-md border border-amber-300 bg-amber-50 p-3 text-amber-900"><AlertTriangle className="w-4 h-4 mt-0.5" />Há múltiplos registros de promoção para este militar. Revise/cancele os incorretos.</div>}<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3"><Info label="Data da promoção atual" value={promocaoAtual?.data_promocao || '—'} /><Info label="Nº/ordem de antiguidade" value={promocaoAtual?.antiguidade_referencia_ordem ?? '—'} /><Info label="DOEMS / Boletim / Ato" value={promocaoAtual?.boletim_referencia || promocaoAtual?.ato_referencia || '—'} /><Info label="Data da publicação" value={promocaoAtual?.data_publicacao || '—'} /><Info label="Status do registro" value={<Badge className={STATUS_BADGE[promocaoAtual?.status_registro || STATUS_ATIVO]}>{promocaoAtual?.status_registro || STATUS_ATIVO}</Badge>} /></div></CardContent></Card>
    <Card><CardHeader className="flex flex-row justify-between items-start gap-3"><CardTitle>Promoções Futuras / Previstas</CardTitle>{canManage && <Button variant="outline" onClick={onOpenPromocaoFuturaModal}>Nova promoção futura</Button>}</CardHeader><CardContent className="space-y-3">{previstos.length === 0 ? <p className="text-sm text-slate-500">Nenhuma promoção futura prevista.</p> : previstos.map((h) => <div key={h.id} className="border rounded-md p-3 space-y-2"><div className="flex gap-2 items-center"><Badge className={STATUS_BADGE[h.status_registro] || STATUS_BADGE.previsto}>{h.status_registro || 'previsto'}</Badge></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 text-sm"><Info label="Posto atual" value={h.posto_graduacao_anterior || '—'} /><Info label="Posto previsto" value={h.posto_graduacao_novo || '—'} /><Info label="Quadro previsto" value={h.quadro_novo || '—'} /><Info label="Data prevista" value={h.data_promocao || '—'} /><Info label="Data da publicação" value={h.data_publicacao || '—'} /><Info label="DOEMS / boletim / ato" value={h.boletim_referencia || h.ato_referencia || '—'} /><Info label="Observações" value={h.observacoes || '—'} /></div>{canManage && <div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => onOpenPromocaoFuturaModal?.(h)}>Editar previsão</Button><Button size="sm" variant="destructive" onClick={() => { setCancelar(h); setErroAcao(''); }}>Cancelar previsão</Button><Button size="sm" onClick={() => { setEfetivar(h); setErroAcao(''); }}>Efetivar promoção</Button></div>}</div>)}</CardContent></Card>
    ...
    <Card><CardHeader className="flex flex-row justify-between items-start gap-3"><CardTitle>Histórico de Promoções</CardTitle>{canManage && <Button size="sm" variant="secondary" onClick={onOpenPromocaoHistoricaModal}>Adicionar promoção anterior</Button>}</CardHeader><CardContent className="space-y-3">{historico.map((h) => <div key={h.id} className="border rounded-md p-3 space-y-2"><div className="flex gap-2 items-center"><Badge className={STATUS_BADGE[h.status_registro] || STATUS_BADGE.pendente}>{h.status_registro || 'pendente'}</Badge>{promocaoAtual?.id === h.id && <Badge variant="outline">Registro atual</Badge>}{isRegistroIncompativel(h) && <Badge variant="outline">Registro incompatível</Badge>}{isRegistroIncompleto(h) && <Badge variant="outline">Registro incompleto</Badge>}</div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 text-sm"><Info label="Posto/graduação anterior" value={h.posto_graduacao_anterior || '—'} /><Info label="Posto/graduação novo" value={h.posto_graduacao_novo || '—'} /><Info label="Quadro" value={h.quadro_novo || '—'} /><Info label="Data da promoção" value={h.data_promocao || '—'} /><Info label="Nº/ordem de antiguidade" value={h.antiguidade_referencia_ordem ?? '—'} /><Info label="DOEMS / boletim / ato" value={h.boletim_referencia || h.ato_referencia || '—'} /><Info label="Observações" value={h.observacoes || '—'} /><Info label="Origem do dado" value={h.origem_dado || '—'} /></div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => setDetalhe(h)}><FileText className="w-4 h-4 mr-1" />Ver detalhes</Button><Button size="sm" variant="outline" disabled={!canManage} onClick={() => { setRetificar(h); setErroAcao(''); }}>Retificar</Button><Button size="sm" variant="destructive" disabled={!canManage} onClick={() => { setCancelar(h); setErroAcao(''); }}>Cancelar</Button></div></div>)}</CardContent></Card>
