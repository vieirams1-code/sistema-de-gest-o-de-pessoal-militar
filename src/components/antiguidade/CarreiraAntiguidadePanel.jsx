...
export default function CarreiraAntiguidadePanel(props) {
  ...
  const podeExcluirPromocao = React.useCallback((registro) => {
    ...
    if (status === STATUS_RETIFICADO && emPromocoesAnteriores) return { permitido: true, motivo: '' };

    return { permitido: false, motivo: 'Somente promoções previstas, promoções anteriores ativas, promoções anteriores canceladas ou promoções anteriores retificadas podem ser excluídas.' };
