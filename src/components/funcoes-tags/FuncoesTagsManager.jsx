...
import { isTagIconKey } from '@/components/tags/TagIcon';
...
export default function FuncoesTagsManager({ canEdit = true, initialTab = 'grupos', showFuncoesTab = true }) {
  ...
  const abrirEditarGrupo = (grupo) => {
    ...
    setFormGrupo({ ...FORM_GRUPO, ...grupo, emoji: grupo?.icon_key || grupo?.emoji || FORM_GRUPO.emoji });
  ...
  const abrirEditarTag = (tag) => {
    ...
    setFormTag({
      ...FORM_TAG,
      ...tag,
      emoji: tag?.icon_key || tag?.emoji || FORM_TAG.emoji,
  ...
  const salvarGrupo = () => {
    const payload = { ...formGrupo, icon_key: isTagIconKey(formGrupo.emoji) ? formGrupo.emoji : null, emoji: isTagIconKey(formGrupo.emoji) ? null : formGrupo.emoji, aplicabilidade: normalizarAplicabilidade(formGrupo.aplicabilidade), ativo: editandoGrupo?.ativo ?? true };
  ...
  const salvarTag = () => {
    ...
    const payload = { ...formTag, icon_key: isTagIconKey(formTag.emoji) ? formTag.emoji : null, emoji: isTagIconKey(formTag.emoji) ? null : formTag.emoji, tipo_visual: tipoVisual, tipo_uso: normalizarTipoUsoTag(formTag.tipo_uso), aplicabilidade: normalizarAplicabilidade(formTag.aplicabilidade), ativo: editandoTag?.ativo ?? true };
