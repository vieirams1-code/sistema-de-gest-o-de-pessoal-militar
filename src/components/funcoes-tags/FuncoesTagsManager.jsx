...
import IconeCatalogo, { OPCOES_ICONE_CATALOGO, OPCOES_ICONE_TAG } from '@/components/funcoes-tags/IconeCatalogo';
...
const FORM_GRUPO = { nome: '', aplicabilidade: 'todos', emoji: '🚒', icon_key: '', cor: '#0F766E', ativo: true };
const FORM_TAG = { grupo_id: '', nome: '', aplicabilidade: 'todos', emoji: '⚠️', icon_key: '', tipo_visual: 'chip', tipo_uso: 'comum', cor: '#F59E0B', ativo: true };
...
export default function FuncoesTagsManager({ canEdit = true, initialTab = 'grupos', showFuncoesTab = true }) {
  ...
  const abrirEditarTag = (tag) => { setEditandoTag(tag); setFormTag({ ...FORM_TAG, ...tag, tipo_uso: tag?.tipo_uso || 'comum', grupo_id: getTagGrupoId(tag) || '' }); setModalTagOpen(true); };
  ...
  const salvarGrupo = () => {
    const payload = { ...formGrupo, icon_key: formGrupo.icon_key || null, aplicabilidade: normalizarAplicabilidade(formGrupo.aplicabilidade), ativo: editandoGrupo?.ativo ?? true };
  ...
  const salvarTag = () => {
    ...
    const payload = { ...formTag, icon_key: formTag.icon_key || null, tipo_visual: tipoVisual, tipo_uso: normalizarTipoUsoTag(formTag.tipo_uso), aplicabilidade: normalizarAplicabilidade(formTag.aplicabilidade), ativo: editandoTag?.ativo ?? true };
  ...
  return (<div className="max-w-6xl mx-auto space-y-6">
    ...
    <EntityModal open={modalGrupoOpen} onClose={cancelarEdicaoGrupo} title={editandoGrupo ? `Editar grupo: ${editandoGrupo.nome}` : 'Novo Grupo de Tags'} description="Defina os dados do grupo de tags." loading={saveGrupo.isPending} onSave={salvarGrupo} preview={<PreviewBadge emoji={formGrupo.emoji} icon_key={formGrupo.icon_key} nome={formGrupo.nome || 'Novo grupo'} cor={formGrupo.cor} />}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><Field label="Nome do grupo"><Input value={formGrupo.nome} onChange={(e) => setFormGrupo({ ...formGrupo, nome: e.target.value })} /></Field><Field label="Aplicabilidade"><AplicabilidadeSelect value={formGrupo.aplicabilidade} onChange={(v) => setFormGrupo({ ...formGrupo, aplicabilidade: v })} /></Field><Field label="Ícone padrão"><EmojiSelect value={formGrupo.emoji} onChange={(emoji) => setFormGrupo({ ...formGrupo, emoji })} /></Field><Field label="Ícone personalizado"><TagIconSelect value={formGrupo.icon_key} onChange={(icon_key) => setFormGrupo({ ...formGrupo, icon_key })} /></Field><Field label="Cor"><ColorInput value={formGrupo.cor} onChange={(cor) => setFormGrupo({ ...formGrupo, cor })} /></Field></div>
    </EntityModal>

    <EntityModal open={modalTagOpen} onClose={cancelarEdicaoTag} title={editandoTag ? `Editar tag: ${editandoTag.nome}` : 'Nova Tag Individual'} description="Configure a tag e seu grupo." loading={saveTag.isPending} onSave={salvarTag} preview={<PreviewBadge emoji={formTag.emoji} icon_key={formTag.icon_key} nome={formTag.nome || 'Nova tag'} cor={formTag.cor} />}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><Field label="Grupo da tag"><select className="w-full h-10 rounded-lg border border-slate-300 px-3 bg-white" value={formTag.grupo_id || ''} onChange={(e) => setFormTag({ ...formTag, grupo_id: e.target.value })}><option value="">Sem grupo</option>{gruposAtivos.map((grupo) => <option key={grupo.id} value={grupo.id}>{grupo.nome}</option>)}</select></Field><Field label="Nome da tag"><Input value={formTag.nome} onChange={(e) => setFormTag({ ...formTag, nome: e.target.value })} /></Field><Field label="Aplicabilidade"><AplicabilidadeSelect value={formTag.aplicabilidade} onChange={(v) => setFormTag({ ...formTag, aplicabilidade: v })} /></Field><Field label="Ícone padrão"><EmojiSelect value={formTag.emoji} onChange={(emoji) => setFormTag({ ...formTag, emoji })} /></Field><Field label="Ícone personalizado"><TagIconSelect value={formTag.icon_key} onChange={(icon_key) => setFormTag({ ...formTag, icon_key })} /></Field><Field label="Tipo visual"><TipoVisualSelect value={formTag.tipo_visual} onChange={(v) => setFormTag({ ...formTag, tipo_visual: v })} /></Field><Field label="Tipo de uso" className="space-y-2"><TipoUsoSelect value={formTag.tipo_uso} onChange={(v) => setFormTag({ ...formTag, tipo_uso: v })} /><TipoUsoHelper tipoUso={formTag.tipo_uso} /></Field><Field label="Cor"><ColorInput value={formTag.cor} onChange={(cor) => setFormTag({ ...formTag, cor })} /></Field></div>
    </EntityModal>
...
function PreviewBadge({ emoji, icon_key, nome, cor, fallback = 'Prévia' }) { return <div className="flex items-center gap-2"><span className="text-xs text-slate-400">Preview visual:</span><span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-sm font-semibold" style={{ borderColor: cor || '#CBD5E1', backgroundColor: `${cor || '#E2E8F0'}22`, color: cor || '#334155' }}>{icon_key ? <TagIcon icon={icon_key} size={16} /> : <IconeCatalogo value={emoji || '🏷️'} />}<span>{nome || fallback}</span></span></div>; }
...
function TipoUsoSelect({ value, onChange }) { return <select className="w-full h-10 rounded-lg border border-slate-300 px-3 bg-white" value={normalizarTipoUsoTag(value)} onChange={(e) => onChange(e.target.value)}><option value="comum">Comum</option><option value="unica">Única</option></select>; }
...
function TagIconSelect({ value, onChange }) {
  return <div className="grid grid-cols-2 gap-2">{OPCOES_ICONE_TAG.map((opt) => <button key={opt.value || 'none'} type="button" onClick={() => onChange(opt.value)} className={`rounded-lg border px-2 py-2 text-xs text-left ${String(value||'')===String(opt.value||'') ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 bg-white'}`}>{opt.value ? <span className="inline-flex items-center gap-2"><TagIcon icon={opt.value} size={14} /> {opt.label}</span> : opt.label}</button>)}</div>;
}
