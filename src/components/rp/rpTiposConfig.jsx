...
export const RP_TIPOS_BASE = [
  ...
  // INTERNO — criado apenas pelo gateway "Descontos em Férias". Oculto da seleção manual do RP.
  {
    ...
    modulo: MODULO_EX_OFFICIO,
    sexo: null,
    descricao: 'Publicação gerada automaticamente pelo módulo Descontos em Férias.',
    palavrasChave: ['desconto', 'ferias', 'dispensa'],
    destaque: false,
    apenasGateway: true,
  },
];

// Tipos internos que NUNCA aparecem na seleção manual do RP (somente via gateway).
export const RP_TIPOS_APENAS_GATEWAY = new Set([
...
function cloneMeta(tipo = {}) {
  return {
    ...
    apenasGateway: Boolean(tipo.apenasGateway),
  };
}
...
export function getTiposRPFiltrados({
  ...
  return Array.from(map.values())
    .filter((t) => {
      ...
      // Tipos internos (gateway) nunca aparecem na seleção manual do RP.
      if (RP_TIPOS_APENAS_GATEWAY.has(t.value) || t.apenasGateway) return false;
