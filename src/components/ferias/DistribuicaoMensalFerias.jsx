    ...
    <>
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-3 mt-5">
        {meses.map((mes) => {
          ...
          const pct = totalPublico ? Math.min(100, Math.round((total / totalPublico) * 100)) : 0;
          return (
            <div key={mes.val} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">{total} militar(es) programado(s)</p>
                </div>
              ...
              {!total && <p className="mt-4 text-xs text-slate-400">Nenhuma definição neste mês.</p>}

              {total > 0 && (
                <div className="mt-4 space-y-4">
                  <GrupoMilitar titulo="Integral · 30 dias" pessoas={integrais} militaresCov={militaresCov} onAbrirMilitar={onAbrirMilitar} />
                  <GrupoMilitar titulo="Fracionado" pessoas={fracionados} militaresCov={militaresCov} onAbrirMilitar={onAbrirMilitar} />
                </div>
              )}
            </div>
          );
...
function GrupoMilitar({ titulo, pessoas, militaresCov, onAbrirMilitar }) {
