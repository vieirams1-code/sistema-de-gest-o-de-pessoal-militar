...
export function filtrarFeriasDoPeriodo(periodo = {}, ferias = []) {
  ...
  return (ferias || []).filter(
    (item) =>
      item &&
      (keys.has(item.periodo_aquisitivo_id) || keys.has(item.periodo_aquisitivo_ref))
  );
}
...
export function calcularDiasGozados(periodo = {}, ferias = []) {
  return filtrarFeriasDoPeriodo(periodo, ferias).reduce((acc, item) => {
    ...
    return STATUS_GOZADA.has(item?.status || '') ? acc + dias : acc;
  }, 0);
}
...
export function calcularDiasPrevistos(periodo = {}, ferias = []) {
  return filtrarFeriasDoPeriodo(periodo, ferias).reduce((acc, item) => {
    const dias = Math.max(0, toNumber(item?.dias, 0));
    return STATUS_PREVISTA.has(item?.status || '') ? acc + dias : acc;
  }, 0);
}
...
export function recalcularSaldoPeriodo(periodo = {}, ferias = []) {
  ...
  return { dias_base, dias_ajuste, dias_total, dias_gozados, dias_previstos, dias_saldo };
}

export function getSaldoConsolidadoPeriodo({ periodo, ferias = [] }) {
  const derivado = recalcularSaldoPeriodo(periodo, ferias);
  return { ...periodo, ...derivado, saldo_disponivel: derivado.dias_saldo };
}
...
export function validarAjusteDiasPeriodo({ periodo = {}, ferias = [], tipo, quantidade }) {
  ...
  if (dias_total_projetado < 0) {
    return { ok: false, mensagem: 'O total de dias do período não pode ficar negativo.' };
  }
  if (dias_saldo_projetado < 0) {
    return { ok: false, mensagem: 'O ajuste deixa o período com saldo inconsistente em relação aos dias já gozados/previstos.' };
  }
  return { ok: true, dias_total_projetado, dias_saldo_projetado, dias_gozados, dias_previstos };
}
