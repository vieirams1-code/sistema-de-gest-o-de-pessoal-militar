...
import { ShieldAlert, Trash2, RefreshCw, AlertTriangle, ChevronDown, ChevronUp, Check, X } from 'lucide-react';
...
export default function AdminCadeiaPanel({ ferias, registrosLivro }) {
  ...
  // Exclusão
  const [confirmarExclusao, setConfirmarExclusao] = useState(null); // { evento, incluirDescendentes }
  ...
  const handleRecalcular = async () => {
    setLoading(true);
    setFeedback(null);
    const resultado = await recalcularCadeiaCompleta({ ferias, cadeia, queryClient });
    setFeedback({ type: 'success', msg: `Cadeia recalculada. Novo status: ${resultado.status}` });
    setLoading(false);
  };
  ...
  const handleConfirmarExclusao = async () => {
    ...
    setFeedback({ type: 'success', msg: 'Exclusão realizada e cadeia recalculada.' });
    setConfirmarExclusao(null);
    setLoading(false);
  };

  const handleCancelarExclusao = () => setConfirmarExclusao(null);

  // Preview do impacto da exclusão
  const previewExclusao = confirmarExclusao
    ? (() => {
        ...
        return { descendentes, idsAfetados, sobreviventes };
      })()
    : null;
    ...
    <div className="mt-4 rounded-xl border border-red-200 bg-red-50/40 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-red-700 uppercase tracking-wide">Administração da Cadeia</span>
          <span className="text-[10px] text-red-400 font-semibold bg-red-100 px-1.5 py-0.5 rounded">ADMIN</span>
        </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-4">
          ...
          {/* Ação: Recalcular Cadeia */}
          <div className="bg-white rounded-lg border border-slate-200 p-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-slate-700">Recalcular Cadeia</p>
                <p className="text-xs text-slate-500 mt-0.5">Reconstrói status, saldo e período aquisitivo sem excluir nada.</p>
              </div>
              <Button
                size="sm"
                variant="outline"
          ...
          {confirmarExclusao && previewExclusao && (
            <div className="bg-red-50 border border-red-300 rounded-lg p-3 space-y-3">
              ...
              <div className="space-y-1">
                <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Evento alvo:</p>
                <div className="bg-white rounded border border-red-200 px-3 py-1.5 text-xs text-slate-700">
                  <strong>{confirmarExclusao.evento.tipo_registro}</strong> — {formatDate(confirmarExclusao.evento.data_registro)}
                </div>
              </div>

              {confirmarExclusao.incluirDescendentes && previewExclusao.descendentes.length > 0 && (
                <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Descendentes que também serão excluídos:</p>
                  <div className="space-y-1">
                    {previewExclusao.descendentes.map(d => (
                      <div key={d.id} className="bg-white rounded border border-red-200 px-3 py-1 text-xs text-red-700">
              ...
              <div className="space-y-1">
                <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Eventos que sobrarão: {previewExclusao.sobreviventes.length}</p>
                {previewExclusao.sobreviventes.length === 0 ? (
                  ...
                  <div className="space-y-1">
                    {previewExclusao.sobreviventes.map(s => (
                      <div key={s.id} className="bg-emerald-50 rounded border border-emerald-200 px-3 py-1 text-xs text-emerald-700">
              ...
              <div className="flex gap-2 pt-1">
                ...
                >
                  {loading ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-1" /> : <Trash2 className="w-4 h-4 mr-1" />}
                  Confirmar Exclusão
                </Button>
                <Button size="sm" variant="outline" onClick={handleCancelarExclusao} disabled={loading}>
                  <X className="w-4 h-4" />
                </Button>
              </div>
            </div>
          ...
          {/* Lista de eventos com ações de exclusão */}
          {!confirmarExclusao && cadeia.length > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Excluir Evento da Cadeia</p>
              {cadeia.map((evento, idx) => {
                const descendentes = identificarDescendentes(evento, cadeia);
                const temDescendentes = descendentes.length > 0;

                return (
                  <div key={evento.id} className="bg-white rounded-lg border border-slate-200 px-3 py-2 flex items-center justify-between gap-2">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-slate-400 font-semibold">#{idx + 1}</span>
                        <span className="text-xs font-semibold text-slate-700 truncate">{evento.tipo_registro}</span>
                      </div>
                      <p className="text-xs text-slate-400">{formatDate(evento.data_registro)}{evento.dias ? ` · ${evento.dias}d` : ''}</p>
                    </div>
                      ...
                      {temDescendentes ? (
                        <>
                          <Button
                            ...
                            title="Excluir apenas este evento"
              ...
              {cadeia.length === 0 && (
                <p className="text-xs text-slate-400 italic">Nenhum evento na cadeia para excluir.</p>
              )}
            </div>
          )}
          ...
          <div className="flex items-start gap-2 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
            ...
            <span>Ações administrativas. Use somente em ambiente de teste ou para corrigir inconsistências reais. Toda exclusão é irreversível.</span>
          </div>
        </div>
      )}
    </div>
