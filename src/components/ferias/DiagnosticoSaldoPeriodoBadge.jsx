...
} from '@/services/diagnosticoSaldoFeriasService';
...
export default function DiagnosticoSaldoPeriodoBadge({
  ...
  const diagnostico = useMemo(() => {
    ...
    return compararSaldoPeriodo({
      periodo,
      ajustes: [...(ajustes || []), ...ajustesVirtuaisLegados],
      ferias,
    });
  ...
  const consistente = Number(diagnostico.diferenca) === 0;
  const badgeClass = consistente
    ? 'border-emerald-200 bg-emerald-100 text-emerald-700'
    : Math.abs(Number(diagnostico.diferenca)) <= 2
      ? 'border-amber-200 bg-amber-100 text-amber-800'
      : 'border-red-200 bg-red-100 text-red-700';
  const Icon = consistente ? CheckCircle2 : AlertTriangle;

  return (
    <div className={`mb-4 rounded-lg border px-3 py-2 text-xs ${consistente ? 'border-emerald-100 bg-emerald-50/70' : 'border-amber-100 bg-amber-50/70'}`}>
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        ...
        <Badge className={`${badgeClass} border text-xs`}>
          ...
          {consistente ? 'Consistente' : 'Divergência de saldo'}
        </Badge>
      </div>
      <div className="grid grid-cols-3 gap-2 text-slate-600">
        <div>
          <p className="text-slate-500">Saldo atual</p>
          <p className="font-semibold text-slate-800">{formatarDias(diagnostico.saldo_atual_sistema)}</p>
        </div>
        <div>
          <p className="text-slate-500">Saldo derivado</p>
          <p className="font-semibold text-slate-800">{formatarDias(diagnostico.saldo_derivado)}</p>
        </div>
        <div>
          <p className="text-slate-500">Diferença</p>
          <p className={`font-semibold ${consistente ? 'text-emerald-700' : 'text-amber-800'}`}>
            {formatarDias(diagnostico.diferenca)}
          </p>
        </div>
      </div>
    </div>
