...
function getTipoIconConfig(grupo, tipo) {
  if (grupo === 'Férias') {
    if (tipo === 'Início') return { Icon: ArrowRightCircle, iconClassName: 'text-emerald-600' };
    if (tipo === 'Término') return { Icon: ArrowLeftCircle, iconClassName: 'text-blue-600' };
    if (tipo === 'Interrupção') return { Icon: PauseCircle, iconClassName: 'text-amber-600' };
    if (tipo === 'Continuação') return { Icon: PlayCircle, iconClassName: 'text-indigo-600' };
  }

  if (grupo === 'Atestado') {
    return { Icon: FileWarning, iconClassName: 'text-rose-600' };
  }

  return { Icon: FileText, iconClassName: 'text-slate-600' };
}
            ...
            <div className="flex items-start gap-4 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 text-sm text-slate-600">
                  ...
                  {tipoLabel && (() => {
                    const tipoIconConfig = getTipoIconConfig(grupoLabel, tipoLabel);
                    const TipoIcon = tipoIconConfig.Icon;
                    return (
                      <Badge className={`border ${getActBadgeClasses(grupoLabel, tipoLabel)} inline-flex items-center gap-1.5`}>
                        <TipoIcon className={`w-3.5 h-3.5 ${tipoIconConfig.iconClassName}`} />
