...
function filterAfastamentos(afastamentos, { militarFilter, tipoFilter, origemFilter }) {
...
function computeTotaisPorOrigem(afastamentos) {
...
function AfastamentosHeader({ count }) {
  return (
...
function AfastamentosStats({ total, totaisPorOrigem }) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
      ...
      {Object.entries(totaisPorOrigem).map(([origem, totalOrigem]) => (
        <div key={origem} className="rounded-lg border border-slate-200 p-3">
          <p className="text-xs text-slate-500">{origem}</p>
          <p className="text-xl font-bold text-slate-800">{totalOrigem}</p>
        </div>
      ))}
    </div>
  );
...
function AfastamentosFilterBar({
  militarFilter,
  setMilitarFilter,
  tipoFilter,
  setTipoFilter,
          ...
          {items.length === 0 && (
            <tr>
              <td colSpan={7} className="py-8 text-center text-slate-500">
                Nenhum afastamento vigente para os filtros aplicados.
              </td>
            </tr>
          )}
          {items.map((item) => (
...
export default function AfastamentosVigentesPanel({ atestados = [], registrosLivro = [], enabled = true }) {
  ...
  const afastamentosFiltrados = useMemo(
    () => filterAfastamentos(afastamentos, { militarFilter, tipoFilter, origemFilter }),
    [afastamentos, militarFilter, tipoFilter, origemFilter],
  );

  const totaisPorOrigem = useMemo(() => computeTotaisPorOrigem(afastamentos), [afastamentos]);

  if (!isReady) return null;

  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
      <AfastamentosHeader count={afastamentosFiltrados.length} />

      <AfastamentosStats total={afastamentos.length} totaisPorOrigem={totaisPorOrigem} />

      <AfastamentosFilterBar
        militarFilter={militarFilter}
        setMilitarFilter={setMilitarFilter}
      ...
      <AfastamentosTable items={afastamentosFiltrados} />
    </div>
  );
}