...
describe('Plano de Férias Institucional — integração da tela e vínculos', () => {
  ...
  it('restringe arquivamento e desarquivamento ao Modo Admin', () => {
    ...
    assert.match(paginaPlanosSource, /origem_plano_ferias/);
    ...
    assert.match(portalServicosSource, /PLANO_INSTITUCIONAL_DESARQUIVAR/);
  });
  ...
  it('mantém um contrato compatível e auditável para a reabertura de campanhas', () => {
    ...
    assert.match(paginaPlanosSource, /const invocarAcaoStatusPlano = async \(payload\) =>/);
